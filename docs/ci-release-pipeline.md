...
- production external evidence action queue probe, proving the operator queue preserves the pending local-mail action before post-dispatch evidence, reduces tracked work to the three external evidence areas after post-dispatch evidence, and exposes owner response bundle directory/zip semantic-preflight and auto-acceptance commands at queue and item level without sending mail or accepting evidence.
...
`production-external-evidence-semantic-preflight-probe-manifest.json` is generated before the final handoff export refresh and release risk policy. It runs the read-only semantic preflight over missing evidence, complete contract-shaped external roots, complete owner response bundles, one-file-missing bundles, and complete-but-fixture/template-shaped bundles, proving returned evidence can be diagnosed as pending, candidate-ready, or owner-repair-needed before auto acceptance runs. Operators inspect `readyForAcceptanceCandidate`, `semanticPreflightStatus`, and `semanticFailCount`; this preflight does not copy returned evidence, accept evidence, or promote fixtures.
