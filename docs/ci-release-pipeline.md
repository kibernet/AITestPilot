...
The documentation freshness contract keeps the pipeline step index, core artifact names, source files, and source-manifest lists aligned. The release evidence index enforces 133 semantic field checks with field-level definition SHA256 and source script SHA256 binding. In the default package boundary, the handoff reports nine missing evidence files from `production-external-evidence-inbox-manifest.json` rather than promoting fixture evidence.
