...
- production external evidence acceptance contract probe, proving the stable repo-side acceptance entry point accepts complete driver, Lua, and live-smoke evidence without promoting fixture data.
- production external evidence acceptance failure probe, proving fully missing and driver-only partial evidence cannot satisfy the stable acceptance entry point under `-RequireAllEvidence`.
