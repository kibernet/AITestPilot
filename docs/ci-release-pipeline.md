...
| 81 | `production_external_evidence_semantic_preflight_probe` |
| 82 | `production_handoff_export_final_refresh` |
| 83 | `production_handoff_export_zip_index` |
| 84 | `release_docs_freshness` |
| 85 | `production_hard_mode_failure_probe` |
| 86 | `production_hard_mode_success_contract_probe` |
| 87 | `release_risk_policy` |
| 88 | `release_evidence_index` |
| 89 | `release_gate` |
| 90 | `release_gate_failure_probe` |
...
- production handoff owner response bundle kit, producing a fillable owner-return template zip with contact roster, driver/Lua/live-smoke evidence directories, required-file manifests, verification/import helpers, and a request draft without sending email or accepting evidence.
- production handoff export refresh, re-running the export after the owner response bundle kit workflow probe so the final owner-facing zip includes the fillable owner response bundle kit and its returned folder/zip auto-acceptance instructions.
- production external evidence action queue probe, proving the operator queue preserves the pending local-mail action before post-dispatch evidence, reduces tracked work to the three external evidence areas after post-dispatch evidence, and exposes owner response bundle directory/zip auto-acceptance commands at queue and item level without sending mail or accepting evidence.
- production external evidence gap analysis, converting the post-dispatch action queue into a machine-readable operator/audit summary with three external gaps, nine missing files, eleven blockers, zero repo-side-closable gaps, and the next owner/operator commands.
...
- production external evidence semantic preflight probe, proving returned owner bundles are read-only checked for missing files, candidate-ready contract shape, and fixture/template semantic failures before acceptance runs.
...
`production-external-evidence-semantic-preflight-probe-manifest.json` is generated before the final handoff export refresh and release risk policy. It runs the read-only semantic preflight over missing evidence, complete contract-shaped external roots, complete owner response bundles, one-file-missing bundles, and complete-but-fixture/template-shaped bundles, proving returned evidence can be diagnosed as pending, candidate-ready, or owner-repair-needed before acceptance runs.
