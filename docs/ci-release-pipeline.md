...
- release progress notification outbox, preparing the requested big-node status email for the progress recipient, writing a `BIG_NODE_ONLY` cadence policy that suppresses separate small proof/probe emails, and keeping local agently-cli authorization plus two-stage send confirmation outside CI.
