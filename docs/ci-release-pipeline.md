...
Use the release wrapper when CI needs one command with stable artifacts and exit-code behavior.

```powershell
.\tools\Invoke-AITestPilotReleasePipeline.ps1
```

For local developers, use the preflight wrapper to run the same release-oriented checks plus the release pipeline in one pass:

```powershell
.\tools\Invoke-AITestPilotReleasePreflight.ps1
```

By default, it runs the local release preflight and then the full release pipeline. Use this for a quick local-only smoke:
...
```powershell
.\tools\Invoke-AITestPilotReleasePreflight.ps1 -SkipReleasePipeline -SummaryPath Temp\release-preflight-summary.json
...
- production handoff package generation, consolidating driver, Lua, live-model, and CI hard-mode host-project next steps without promoting fixture evidence, with generated-content quality checks, a blocker-resolution map, per-owner action packets, a runnable external-evidence preflight script, and a unified direct-fallback acceptance wrapper.
- production handoff external evidence preflight probe, proving that generated preflight script accepts complete host-project-shaped fixture evidence with `-RequireAllEvidence -RunIntake` and that the generated direct-fallback acceptance wrapper writes a validated Markdown report while preserving the fixture boundary.
- production external evidence inbox, generating the returned-evidence directory layout and bundled/direct-inbox bridge wrapper for driver, Lua, and live-smoke evidence while the canonical owner-return flow stays owner-return status, semantic preflight, then auto acceptance with `-RequireAllEvidence`.
- production external evidence inbox contract probe, proving the returned-evidence inbox bridge wrapper accepts complete host-project-shaped fixture evidence while preserving the fixture boundary.
- production handoff export, creating a compact owner-facing export folder and zip with the handoff package, owner packets, generated kits, owner-return status, operator actions, and contract reports.
- production handoff status, summarizing accepted versus pending owner packets, remaining blockers, required evidence files, and next owner-return status, semantic-preflight, and auto-acceptance commands.
...
- production handoff owner input request pack, turning the remaining owner contacts, pending dispatches, pending packets, returned-evidence files, owner response bundle routes, required-files paths, zip owner-return-status commands, zip semantic-preflight commands, zip auto-acceptance commands, export helpers, and blocker reasons into an owner-facing request package and progress-recipient email draft without sending mail or accepting evidence.
...
- production handoff owner response bundle kit, producing a fillable owner-return template zip with contact roster, driver/Lua/live-smoke evidence directories, required-file manifests, verification/import helpers, and a request draft that keeps owner-return status before semantic preflight and auto acceptance without sending email or accepting evidence.
- production handoff export refresh, re-running the export after the owner response bundle kit workflow probe so the final owner-facing zip includes the fillable owner response bundle kit, source-to-export SHA256 proof for every kit file, and its returned folder/zip owner-return-status-then-semantic-preflight-then-auto-acceptance instructions.
- production external evidence action queue, writing the canonical operator action queue from the current remaining-work snapshot before the probe runs, while preserving the no-mail, no-real-evidence, and no-fixture-promotion boundaries and exposing owner response bundle directory/zip status commands before semantic preflight and auto acceptance.
- production external evidence action queue probe, proving isolated pending and post-dispatch probe queues preserve the local-mail boundary, reduce tracked work to the three external evidence areas only after post-dispatch evidence, and expose owner response bundle directory/zip owner-return status, semantic-preflight, and auto-acceptance commands at queue and item level without sending mail or accepting evidence.
- production external evidence gap analysis, converting the canonical action queue and current remaining-work snapshot into a machine-readable operator/audit summary with three external gaps, nine missing files, eleven blockers, zero repo-side-closable gaps, and the next owner/operator commands, with owner-return status before semantic preflight.
- production handoff owner route map, tying each external gap to its owner packet, contact/send state, owner response bundle area, `required-files.json`, owner-return status command, semantic preflight command, auto-acceptance command, and hard-validation command without claiming repo-side closure.
...
- production external evidence acceptance contract probe, proving the stable repo-side acceptance implementation delegated by auto acceptance can accept complete driver, Lua, and live-smoke evidence and write a validated Markdown report without promoting fixture data.
- production external evidence acceptance failure probe, proving fully missing and driver-only partial evidence cannot satisfy the delegated/direct-fallback acceptance implementation under `-RequireAllEvidence` while still writing validated rejection reports.
...
- release evidence index, exporting a machine-readable source-manifest summary for CI, portal handoff, and audit consumers, including field-level coverage for 133 semantic fields across the primary release evidence manifests plus field-level definition and source script SHA256 binding plus source manifest SHA256 hash set binding.
...
The release evidence index enforces 133 semantic field checks with field-level definition SHA256 and source script SHA256 binding. In the default package boundary, the handoff reports nine missing evidence files from `production-external-evidence-inbox-manifest.json` rather than promoting fixture evidence.

Owner-return semantic preflight covers a complete owner response bundle zip, partial zip, semantic-bad zip, arbitrary single top-level wrapper zip, unsafe zip, extra-payload owner response bundle, and nested-payload owner response bundle zip. Generate and verify the owner route map with `Invoke-AITestPilotProductionHandoffOwnerRouteMap.ps1` and `Invoke-AITestPilotProductionHandoffOwnerRouteMapProbe.ps1`.
...
`production-external-evidence-inbox-manifest.json` is generated before the release risk policy. It validates `production-external-evidence-inbox`, a returned-evidence directory layout containing driver, Lua, and live-smoke folders plus `accept-returned-evidence.ps1`; the wrapper is a bundled bridge for direct inbox files, a filled owner response bundle directory, or a filled owner response bundle zip, not the primary owner-return checklist path. The canonical operator path is owner-return status, then semantic preflight, then auto acceptance with `-RequireAllEvidence`. In the default package-release path it reports nine missing required files and no accepted real host-project evidence.

`production-external-evidence-inbox-contract-probe-manifest.json` is generated before the release risk policy. It fills the returned-evidence inbox with complete accepted fixture evidence from outside the repository, runs the bundled bridge `accept-returned-evidence.ps1` with contract mode through direct inbox files, owner response bundle directory, and owner response bundle zip inputs, and records acceptance Markdown while preserving `realHostProjectEvidenceAccepted=false` and `releasePipelineUsesFixture=false`.

`production-external-evidence-gap-analysis-manifest.json` is generated before the release risk policy. It summarizes the post-dispatch external evidence queue as three remaining owner areas, nine missing files, eleven blockers, zero repo-side-closable gaps, and the next owner/export, operator owner-return status, semantic-preflight, auto-acceptance, and hard-validation commands while preserving the no-mail, no-real-evidence, and no-fixture-promotion boundaries.
...
`production-handoff-owner-response-bundle-kit-manifest.json` is generated before the release risk policy. It validates the fillable owner response bundle kit, the unified `owner-response-bundle-template`, three per-owner mini kits and zips under `owner-response-mini-kits\`, `merge-owner-mini-kits.ps1`, the verifier, import helper, semantic preflight helper, and the request draft while preserving no-mail, no-real-evidence, and no-fixture-promotion boundaries.
...
`release-risk-policy-manifest.json` is the machine-readable release-blocker decision for downstream CI, portal, or audit tooling. It is generated after the production handoff and hard-mode probes and before the release evidence index, recording whether package release is allowed under the active production driver, production Lua, and live-model enforcement switches. It also records the release risk policy source script SHA256 so the release gate rejects stale policy output generated by an older script.

`production-external-evidence-owner-return-bundle-status-manifest.json` is generated after semantic preflight proof and before the final progress-notification refresh. It is the first read-only check in the canonical owner-return flow over the current handoff status, returned-evidence inbox, owner input request pack, owner response bundle kit, and action queue. With no returned bundle input it stays `ownerReturnReadinessStatus=PENDING_EXTERNAL_EVIDENCE`, `nextRequiredAction=collect_owner_response_bundle_zip`, `pendingOwnerPacketCount=3`, `remainingMissingFileCount=9`, and `remainingBlockingReasonCount=11`. When an operator supplies `-OwnerResponseBundleDir`, `-OwnerResponseBundleZipPath`, `AITESTPILOT_OWNER_RESPONSE_BUNDLE_DIR`, or `AITESTPILOT_OWNER_RESPONSE_BUNDLE_ZIP_PATH`, the script runs semantic preflight only and maps a complete returned zip to `READY_FOR_AUTO_ACCEPTANCE_CANDIDATE` or strict-payload failures to `NEEDS_OWNER_REPAIR`; explicit parameters override environment discovery, and it does not run acceptance, hard validation, mail, or real evidence promotion. Operators then run semantic preflight directly and invoke auto acceptance with `-RequireAllEvidence` only for candidate-ready returns. `production-external-evidence-owner-return-bundle-status-probe-manifest.json` proves five status cases with `caseCount=5` and `checkCount=8`: default pending, explicit complete owner response bundle zip candidate-ready, environment zip candidate-ready, explicit zip overriding environment discovery, and extra-payload owner response bundle repair-needed.

`release-evidence-index.json` is the stable machine-readable summary for downstream CI, portal, or audit tooling. It indexes the primary release-gate source manifests, including the release risk policy, records source status coverage, listed-file coverage, optional live-smoke skip handling, and auxiliary manifest inventory. The release gate validates `release-evidence-index-manifest.json` before allowing release; after the pipeline manifest is written, the pipeline reruns the handoff export and zip index from the copied artifact root, then refreshes the final artifact copy of the index so uploaded evidence also records `pipelineManifestIncluded=true`. It then runs `Invoke-AITestPilotFirstTestableReleaseProbe.ps1` against the copied artifact, runs `Invoke-AITestPilotFirstTestableOperatorDashboard.ps1`, and refreshes the final artifact index again so the uploaded evidence includes `first-testable-release-manifest.json`, `first-testable-release.md`, `first-testable-operator-dashboard-manifest.json`, `first-testable-operator-dashboard.md`, `production-handoff-export\FIRST-TESTABLE.md`, the zip-indexed `production-handoff-export\run-owner-return-status.ps1` entry, the three per-owner mini kit zips, and `merge-owner-mini-kits.ps1`. The wrapper rejects stale final artifacts unless that last index was generated after `first-testable-release-manifest.json`, inventories the first-testable and dashboard manifests as PASS auxiliary manifests with matching SHA256, and proves their listed files include `first-testable-release.md` and `first-testable-operator-dashboard.md` with no missing listed files. `Invoke-AITestPilotFinalArtifactFreshnessProbe.ps1` runs after that final refresh as a read-only artifact check; it writes outside `artifacts\ai-testpilot-release\latest` so the probe itself cannot change the indexed artifact, and it is the standalone command to run after any manual first-testable rerun plus dashboard generation plus final index refresh. If the wrapper fails before that final refresh, `artifacts\ai-testpilot-release\latest` keeps the failing `pipeline-manifest.json`, removes final release gate/risk/index status files from the copied artifact, and writes `release-artifact-invalidated-manifest.json`.

`tools\Invoke-AITestPilotFirstTestableReleaseProbe.ps1` is a post-pipeline operator check for the copied `artifacts\ai-testpilot-release\latest` directory. It writes `first-testable-release-manifest.json` and `first-testable-release.md`, verifies the PASS pipeline/gate/risk/index state, checks the owner-facing export zip SHA256 against `production-handoff-export-zip-index-manifest.json`, confirms the operator entry points under `production-handoff-export\`, requires the same entry points to appear in the zip index, requires three per-owner mini kit zips plus the mini-kit merge helper, and keeps the first-testable boundary explicit: three external owner areas, nine missing files, eleven blockers, one local progress-mail action, no sent email, no real host-project evidence accepted, and no fixture promotion. `tools\Invoke-AITestPilotFirstTestableOperatorDashboard.ps1` is the follow-up read-only operator dashboard for the same copied artifact; it writes `first-testable-operator-dashboard-manifest.json` and `first-testable-operator-dashboard.md`, summarizes the next owner-return status, semantic-preflight, and acceptance-after-preflight commands, and preserves the blocked owner-contact/send boundary. Because these probes rewrite first-testable artifacts when run standalone, manually rerun `Invoke-AITestPilotReleaseEvidenceIndex.ps1 -EvidenceBundleDir .\artifacts\ai-testpilot-release\latest` before `Invoke-AITestPilotFinalArtifactFreshnessProbe.ps1 -ArtifactDir .\artifacts\ai-testpilot-release\latest`.
