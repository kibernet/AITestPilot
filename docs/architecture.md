...
- `release-risk-policy-manifest.json` and `release-risk-policy.md`: aggregated release-risk policy evidence for AI exploration, unresolved high-risk graph nodes, production driver evidence, production Lua evidence, live endpoint policy, and CI provider controls.
...
- production Lua patch readiness manifest and bound failure probe proving real production Lua patch evidence is required when production Lua patching is a hard release condition.
