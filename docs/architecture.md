...
- `release-risk-policy-manifest.json` and `release-risk-policy.md`: aggregated release-risk policy evidence for AI exploration, unresolved high-risk graph nodes, production driver evidence, production Lua evidence, live endpoint configuration and policy, and CI provider controls.
...
- production handoff package manifest plus README, action plan, required-evidence JSON, and CI command examples consolidating the host-project driver, Lua, live-model, and CI hard-mode next steps.
...
- release risk policy manifest proving package release is blocked unless AI exploration, high-risk graph, driver evidence, Lua evidence, live endpoint policy, CI provider controls, production handoff evidence, and hard-mode failure evidence are explicitly accepted.
