...
$sendScriptLines = @(
    ...
    "    `$safe = `$safe.Trim('-')",
    "    if ([string]::IsNullOrWhiteSpace(`$safe)) { return 'owner' }",
    "    return `$safe",
    "}",
    "function Write-OwnerPacketSendReceipt {",
    "    param(",
    "        [object]`$Entry,",
    "        [object]`$Contact,",
    "        [string[]]`$Lines",
    "    )",
    "    if ([string]::IsNullOrWhiteSpace(`$ReceiptDir)) { return }",
    ...
    "    }",
    "    `$safeOwner = Convert-ToSafeFileName ([string]`$Entry.owner)",
    "    `$receiptPath = Join-Path `$ReceiptDir (`$safeOwner + '.json')",
    "    `$receipt = [ordered]@{",
    "        schemaVersion = 'aitestpilot.production_handoff_owner_packet_send_receipt.v1'",
    "        generatedAtUtc = (Get-Date).ToUniversalTime().ToString('O')",
    "        owner = [string]`$Entry.owner",
    "        area = [string]`$Entry.area",
    "        recipient = [string]`$Contact.emailAddress",
    "        subject = [string]`$Entry.subject",
    "        bodyFile = [string]`$Entry.bodyFile",
    "        attachment = '.\production-handoff-export.zip'",
    "        confirmationTokenSupplied = [bool]`$Send",
    ...
    "        }",
        ...
        "            if (`$PrepareConfirmation -and -not `$Send -and `$sendExitCode -eq 8) {",
        "                `$exitCodeToReturn = 8",
        "                continue",
        "            }",
        "            exit `$sendExitCode",
        "        }",
        "        if (`$Send) { Write-OwnerPacketSendReceipt `$entry `$contact `$sendOutput }",
    "    }",
    "    if (`$exitCodeToReturn -ne 0) { exit `$exitCodeToReturn }",
    "}",
    "finally {",
    "    Pop-Location",
    "}"
...
$readmeLines = @(
    ...
    "5. After operator approval, pass those tokens through ``-ConfirmationTokenMapPath`` with ``-Send``; optionally add ``-ReceiptDir`` to write one receipt per owner.",
    "",
...
$sendScriptContentValidated = $sendScriptText.Contains("agently-cli auth status") -and
    ...
    $sendScriptText.Contains("Write-OwnerPacketSendReceipt") -and
    $sendScriptText.Contains("production_handoff_owner_packet_send_receipt.v1") -and
    $sendScriptText.Contains("ReceiptDir") -and
