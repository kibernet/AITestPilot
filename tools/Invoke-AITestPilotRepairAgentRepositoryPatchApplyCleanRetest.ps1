...
if ([string]::IsNullOrWhiteSpace($ProbeBundleDir)) {
    $ProbeBundleDir = Join-Path $repoRoot "Temp\release-evidence\repository-patch-clean-apply-retest-probe"
}

if ([string]::IsNullOrWhiteSpace($ProbeRepoDir)) {
    $ProbeRepoDir = Join-Path $repoRoot "Temp\repository-patch-clean-apply-retest-probe-repo"
}

if ([string]::IsNullOrWhiteSpace($ManifestPath)) {
    $ManifestPath = Join-Path $EvidenceBundleDir "repair-agent-repository-patch-apply-clean-retest-manifest.json"
}
...
if ($patchOutputManifest.status -ne "PASS" -or
    ...
    throw "Clean apply/retest probe patch output import did not prove completed external-agent provenance."
}
...
Invoke-GitChecked @("-C", $probeRepoPath, "commit", "-m", "seed clean apply retest probe") | Out-Null
...
if (-not (Test-Path $guardManifestPath)) {
    throw "Clean apply/retest guard manifest was not produced: $guardManifestPath"
}
...
if ($guardManifest.status -ne "PASS" -or
    ...
    throw "Clean apply/retest guard manifest did not prove repository patch application and rollback patch generation."
}
...
if (-not $patchedFileContainsExpectedFix) {
    throw "Clean apply/retest probe patched file does not include expected null guard."
}
...
if ($postApplyRetestInvoked) {
    ...
    if (-not (Test-Path $postApplyRetestManifestPath)) {
        throw "Post-apply retest manifest was not produced: $postApplyRetestManifestPath"
    }
    ...
    if ($postApplyRetestManifest.status -ne "PASS" -or
        ...
        throw "Post-apply retest did not pass before rollback."
    }

    Copy-Item -LiteralPath $postApplyRetestManifestPath `
        -Destination (Join-Path $evidenceBundlePath "repair-agent-repository-patch-clean-apply-retest-repair-retest-manifest.json") `
        -Force
}
...
if (-not $worktreeCleanAfterRollback) {
    throw "Clean apply/retest probe repository was not clean after rollback."
}

$guardManifestTarget = Join-Path $evidenceBundlePath "repair-agent-repository-patch-clean-apply-retest-guard-manifest.json"
$rollbackPatchTarget = Join-Path $evidenceBundlePath "repair-agent-repository-patch-clean-apply-retest-rollback.patch"
$rollbackPlanTarget = Join-Path $evidenceBundlePath "repair-agent-repository-patch-clean-apply-retest-rollback-plan.md"
$preflightTarget = Join-Path $evidenceBundlePath "repair-agent-repository-patch-clean-apply-retest-preflight-manifest.json"
$patchTarget = Join-Path $evidenceBundlePath "repair-agent-repository-patch-clean-apply-retest.patch"
...
$files = @(
    "repair-agent-repository-patch-clean-apply-retest-guard-manifest.json",
    "repair-agent-repository-patch-clean-apply-retest-preflight-manifest.json",
    "repair-agent-repository-patch-clean-apply-retest-rollback.patch",
    "repair-agent-repository-patch-clean-apply-retest-rollback-plan.md",
    "repair-agent-repository-patch-clean-apply-retest.patch"
)

if ($postApplyRetestInvoked) {
    $files += "repair-agent-repository-patch-clean-apply-retest-repair-retest-manifest.json"
}

$manifest = [ordered]@{
    schemaVersion = "aitestpilot.repair_agent_repository_patch_apply_clean_retest.v1"
    status = "PASS"
...
Write-Output "Repair agent repository patch clean apply/retest manifest: $manifestPath"
Write-Output "PASS AI TestPilot repair agent repository patch clean apply/retest"
