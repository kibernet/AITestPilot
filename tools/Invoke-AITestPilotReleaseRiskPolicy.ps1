...
function Test-ProductionHandoffOwnerMissingEvidenceSplit {
    ...
    foreach ($area in $expectedMissingFilesByArea.Keys) {
        ...
        if (Convert-ToBool (Get-JsonValue $ownerStatus "acceptedExternalEvidence" $true)) {
            return $false
        }
...
$productionDriverBindingKitManifest = Read-PolicyJson "production-driver-binding-kit-manifest.json" "Production driver binding kit manifest"
...
$productionDriverBindingKitExportHelperAccepted = (
    $null -ne $productionDriverBindingKitManifest -and
    $productionDriverBindingKitManifest.status -eq "PASS" -and
    (Get-JsonValue $productionDriverBindingKitManifest "schemaVersion" "") -eq "aitestpilot.production_driver_binding_kit_probe.v1" -and
    (Convert-ToBool (Get-JsonValue $productionDriverBindingKitManifest "kitGenerated" $false)) -and
    (Convert-ToInt (Get-JsonValue $productionDriverBindingKitManifest "generatedFileCount" 0)) -ge 6 -and
    (Test-ContainsAll -Actual (Convert-ToArray (Get-JsonValue $productionDriverBindingKitManifest "generatedFiles" @())) -Required @("production-driver-binding-kit/Export-ProductionDriverEvidenceBundle.ps1")) -and
    (Convert-ToBool (Get-JsonValue $productionDriverBindingKitManifest "exportHelperGenerated" $false)) -and
    (Convert-ToBool (Get-JsonValue $productionDriverBindingKitManifest "exportHelperRequiresProductionBoundReadiness" $false)) -and
    (Convert-ToBool (Get-JsonValue $productionDriverBindingKitManifest "exportHelperRejectedSampleUnboundEvidence" $false)) -and
    (Convert-ToBool (Get-JsonValue $productionDriverBindingKitManifest "exportRejectionReadinessRejectedCurrentSample" $false)) -and
    (Convert-ToInt (Get-JsonValue $productionDriverBindingKitManifest "exportRejectionBlockingReasonCount" 0)) -ge 5 -and
    (Convert-ToBool (Get-JsonValue $productionDriverBindingKitManifest "generatedKitOnly" $false)) -and
    -not (Convert-ToBool (Get-JsonValue $productionDriverBindingKitManifest "readyForProductionDriverRelease" $true)) -and
...
Add-PolicyCheck "production_driver_binding_kit_export_helper_policy" $productionDriverBindingKitExportHelperAccepted `
    "Production driver binding kit must include an export helper that only packages production-bound driver evidence and rejects the current sample/unbound evidence." `
    "production_driver_binding_kit_export_helper_not_accepted"
...
$productionHandoffExportAccepted = (
    ...
    (Convert-ToBool (Get-JsonValue $productionHandoffExportManifest "semanticPreflightProbeOwnerResponseBundleZipReady" $false)) -and
    (Convert-ToBool (Get-JsonValue $productionHandoffExportManifest "semanticPreflightProbeOwnerResponseBundleZipArbitraryWrapperReady" $false)) -and
    (Convert-ToBool (Get-JsonValue $productionHandoffExportManifest "semanticPreflightProbeUnsafeOwnerResponseBundleZipRejected" $false)) -and
...
$productionHandoffStatusAccepted = (
    ...
    (Convert-ToInt (Get-JsonValue $productionHandoffStatusManifest "totalBlockingReasonCount" 0)) -eq 11 -and
    (Convert-ToInt (Get-JsonValue $productionHandoffStatusManifest "remainingBlockingReasonCount" -1)) -eq 11 -and
    (Convert-ToInt (Get-JsonValue $productionHandoffStatusManifest "remainingMissingFileCount" -1)) -eq 9 -and
    ...
    (Convert-ToInt (Get-JsonValue $productionHandoffStatusManifest "checkCount" 0)) -eq 9 -and
...
Add-PolicyCheck "production_handoff_status_policy" $productionHandoffStatusAccepted `
    "Production handoff evidence must include an owner-level external evidence collection status report with nine missing files and without promoting fixture data." `
...
$productionExternalEvidenceSemanticPreflightProbeAccepted = (
    ...
    (Convert-ToBool (Get-JsonValue $productionExternalEvidenceSemanticPreflightProbeManifest "ownerResponseBundleZipReady" $false)) -and
    (Convert-ToBool (Get-JsonValue $productionExternalEvidenceSemanticPreflightProbeManifest "ownerResponseBundleZipArbitraryWrapperReady" $false)) -and
    (Convert-ToBool (Get-JsonValue $productionExternalEvidenceSemanticPreflightProbeManifest "unsafeOwnerResponseBundleZipRejected" $false)) -and
...
$productionExternalEvidenceOwnerReturnBundleStatusAccepted = (
    ...
    (Get-JsonValue $productionExternalEvidenceOwnerReturnBundleStatusManifest "ownerReturnNextStep" "") -eq "run_owner_return_status_then_semantic_preflight_then_auto_acceptance" -and
    (Convert-ToBool (Get-JsonValue $productionExternalEvidenceOwnerReturnBundleStatusManifest "ownerReturnNextStepSequenceAligned" $false)) -and
    (Convert-ToInt (Get-JsonValue $productionExternalEvidenceOwnerReturnBundleStatusManifest "ownerReturnStatusCommandCount" 0)) -eq 3 -and
    (Convert-ToInt (Get-JsonValue $productionExternalEvidenceOwnerReturnBundleStatusManifest "ownerReturnZipStatusCommandCount" 0)) -eq 3 -and
    (Convert-ToInt (Get-JsonValue $productionExternalEvidenceOwnerReturnBundleStatusManifest "ownerReturnSemanticPreflightCommandCount" 0)) -eq 3 -and
    (Convert-ToInt (Get-JsonValue $productionExternalEvidenceOwnerReturnBundleStatusManifest "ownerReturnZipSemanticPreflightCommandCount" 0)) -eq 3 -and
    (Convert-ToInt (Get-JsonValue $productionExternalEvidenceOwnerReturnBundleStatusManifest "ownerReturnAutoAcceptanceCommandCount" 0)) -eq 3 -and
    (Convert-ToInt (Get-JsonValue $productionExternalEvidenceOwnerReturnBundleStatusManifest "ownerReturnZipAutoAcceptanceCommandCount" 0)) -eq 3 -and
    (Convert-ToInt (Get-JsonValue $productionExternalEvidenceOwnerReturnBundleStatusManifest "legacyAcceptanceWrapperCommandPreservedCount" 0)) -eq 3 -and
    ...
    (Convert-ToInt (Get-JsonValue $productionExternalEvidenceOwnerReturnBundleStatusManifest "checkCount" 0)) -eq 9 -and
...
$manifest = [ordered]@{
    ...
    productionExternalEvidenceSemanticPreflightSemanticBadBundleZipRejected = (Get-JsonValue $productionExternalEvidenceSemanticPreflightProbeManifest "semanticBadBundleZipRejected" $false)
    ...
    productionExternalEvidenceOwnerReturnBundleNextStep = (Get-JsonValue $productionExternalEvidenceOwnerReturnBundleStatusManifest "ownerReturnNextStep" "")
    productionExternalEvidenceOwnerReturnBundleNextStepSequenceAligned = (Get-JsonValue $productionExternalEvidenceOwnerReturnBundleStatusManifest "ownerReturnNextStepSequenceAligned" $false)
    productionExternalEvidenceOwnerReturnBundleStatusCommandCount = (Convert-ToInt (Get-JsonValue $productionExternalEvidenceOwnerReturnBundleStatusManifest "ownerReturnStatusCommandCount" 0))
    productionExternalEvidenceOwnerReturnBundleZipStatusCommandCount = (Convert-ToInt (Get-JsonValue $productionExternalEvidenceOwnerReturnBundleStatusManifest "ownerReturnZipStatusCommandCount" 0))
    productionExternalEvidenceOwnerReturnBundleSemanticPreflightCommandCount = (Convert-ToInt (Get-JsonValue $productionExternalEvidenceOwnerReturnBundleStatusManifest "ownerReturnSemanticPreflightCommandCount" 0))
    productionExternalEvidenceOwnerReturnBundleZipSemanticPreflightCommandCount = (Convert-ToInt (Get-JsonValue $productionExternalEvidenceOwnerReturnBundleStatusManifest "ownerReturnZipSemanticPreflightCommandCount" 0))
    productionExternalEvidenceOwnerReturnBundleAutoAcceptanceCommandCount = (Convert-ToInt (Get-JsonValue $productionExternalEvidenceOwnerReturnBundleStatusManifest "ownerReturnAutoAcceptanceCommandCount" 0))
    productionExternalEvidenceOwnerReturnBundleZipAutoAcceptanceCommandCount = (Convert-ToInt (Get-JsonValue $productionExternalEvidenceOwnerReturnBundleStatusManifest "ownerReturnZipAutoAcceptanceCommandCount" 0))
    productionExternalEvidenceOwnerReturnBundleLegacyAcceptanceWrapperCommandPreservedCount = (Convert-ToInt (Get-JsonValue $productionExternalEvidenceOwnerReturnBundleStatusManifest "legacyAcceptanceWrapperCommandPreservedCount" 0))
...
$reportLines = @(
    ...
    "- Production handoff export semantic preflight partial bundle zip rejected: $($manifest.productionHandoffExportSemanticPreflightProbePartialBundleZipRejected)",
    ...
    "- Production external evidence semantic preflight check count: $($manifest.productionExternalEvidenceSemanticPreflightCheckCount)",
    ...
    "- Production external evidence semantic preflight partial bundle zip rejected: $($manifest.productionExternalEvidenceSemanticPreflightPartialBundleZipRejected)",
    ...
    "- Production external evidence owner return bundle next step: $($manifest.productionExternalEvidenceOwnerReturnBundleNextStep)",
    "- Production external evidence owner return bundle command sequence aligned: $($manifest.productionExternalEvidenceOwnerReturnBundleNextStepSequenceAligned)",
    "- Production external evidence owner return bundle status commands: $($manifest.productionExternalEvidenceOwnerReturnBundleStatusCommandCount)",
    "- Production external evidence owner return bundle semantic preflight commands: $($manifest.productionExternalEvidenceOwnerReturnBundleSemanticPreflightCommandCount)",
    "- Production external evidence owner return bundle auto acceptance commands: $($manifest.productionExternalEvidenceOwnerReturnBundleAutoAcceptanceCommandCount)",
